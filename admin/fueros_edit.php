<? 
			...
			{ 
				if(isset($_POST["id"])&&is_numeric($_POST["id"])) 
				{
					...
					$sql = "update fueros set fuero = '".$_POST['fuero']."',activo = '".$_POST['activo']."' where id = ".$_POST["id"];
					$conn->execute($sql); 
					$id = $_POST["id"];
				...
				{ 
					$sql = "insert into fueros (fuero,activo) values ('".$_POST['fuero']."','".$_POST['activo']."')"; 
					$conn->execute($sql); 
				...
				{
					header("Location: fueros.php?p=".$_REQUEST['p']."&orden=".$_REQUEST['orden']."&filtros=".$_REQUEST['filtros']."&filtros_p=".$_REQUEST['filtros_p']);
					die();
				}
				else
				{
			...
			if(isset($_GET["id"])) 
			{ 
				$sql = "select * from fueros where id = ".$_GET["id"]; 
				$rs = $conn->execute($sql); 
                    ...
                    <tr>
                        ...
                        <td align="center" bgcolor="#e5e5e5"><table width='50%' border="0" cellpadding="0" cellspacing="5">
                                <tr>
                                    ...
                                    <td align='center' style="padding-right:15px;"><img onclick='document.location = "fueros.php?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif" border="0" style="cursor:pointer;">
                                    </td>
                    ...
                    <tr>
                        ...
                        <td align="center" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="3" style="border-collapse: collapse;">
                                ...
                                <tr <?=(false == true && $bilingual == false)? "style=\"display: none;\"":"";?>>
                                    ...
                                    <td width="60%" class='arial11' style="background-image:url(images/separador_h1.gif); background-repeat:repeat-x; background-position:bottom; padding:7px;"><input id='fuero'  name='fuero' type='textbox' class='comun' value='<?=(isset($id)?htmlentities($fuero,ENT_QUOTES):"")?>'>
								</td>
                                </tr>
                    ...
                    <tr>
                        ...
                        <td align="center" bgcolor="#e5e5e5"><table width='50%' border="0" cellpadding="0" cellspacing="5">
                                <tr>
                                    ...
                                    <td align='center' style="padding-right:15px;"><img border="0" onclick='document.location = "fueros.php?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif" style="cursor:pointer;">
                                    </td>
                                    <td align='center' style="background-image:url(images/separador_v.gif); background-repeat:repeat-y; background-position:left; padding-left:15px;"><? if(($usr->checkUserRights(4, $id) != 'disabled') || (!$id)) print('<input name="imageField2" type="image" id="imageField2" onclick="javascript:set_type(0);" src="images/btn_guardar.gif">'); ?></td>
                                    <td align='center'><? if(($usr->checkUserRights(4, $id) != 'disabled') || (!$id)) print('<input name="imageField2" type="image" id="imageField2" src="images/btn_guardaryvolver.gif" onclick="javascript:set_type(1);" value=\'Guardar y volver al listado\'>'); ?></td>
                                </tr>

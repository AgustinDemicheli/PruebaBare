<? 
			...
			{ 
				if(isset($_POST["id"])&&is_numeric($_POST["id"])) 
				{
					...
					$sql = "update admin_secciones set nombre = '".$_POST['nombre']."',orden = '".$_POST['orden']."',activo = '".$_POST['activo']."' where id = ".$_POST["id"];
					$conn->execute($sql); 
					$id = $_POST["id"];
				...
				{ 
					$sql = "insert into admin_secciones (nombre,orden,activo) values ('".$_POST['nombre']."','".$_POST['orden']."','".$_POST['activo']."')"; 
					$conn->execute($sql); 
				...
				{
					header("Location: g_admin_seccion.php");
					die();
				}
				else
				{
			...
			if(isset($_GET["id"])) 
			{ 
				$sql = "select * from admin_secciones where id = ".$_GET["id"]; 
				$rs = $conn->execute($sql); 
...
<head>
	...
	function Chk()
	{
		...
		{
			alert("Debe ingresar un nombre de seccion");
      ...
      <tr>
        ...
        <td align="center" bgcolor="#e5e5e5"><table width='50%' border="0" cellpadding="0" cellspacing="5">
          <tr>
            ...
            <td align='center' style="padding-right:15px;"><img onclick='document.location="g_admin_seccion.php?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif" border="0" style="cursor:pointer;">
            </td>
    ...
    <tr>
      ...
      <td align="center" bgcolor="#e5e5e5"><table width='50%' border="0" cellpadding="0" cellspacing="5">
          <tr>
            ...
            <td align='center' style="padding-right:15px;"><img border="0" onclick='document.location="g_admin_seccion.php?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif" style="cursor:pointer;">
            </td>
            <td align='center' style="background-image:url(images/separador_v.gif); background-repeat:repeat-y; background-position:left; padding-left:15px;"><? if(($usr->checkUserRights(4, $id) != 'disabled') || (!$id)) print('<input name="imageField2" type="image" id="imageField2" onclick="javascript:set_type(0);" src="images/btn_guardar.gif">'); ?></td>
            <td align='center'><? if(($usr->checkUserRights(4, $id) != 'disabled') || (!$id)) print('<input name="imageField2" type="image" id="imageField2" src="images/btn_guardaryvolver.gif" onclick="javascript:set_type(1);" value=\'Guardar y volver al listado\'>'); ?></td>
          </tr>

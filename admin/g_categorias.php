<? 
	...
	if(is_array($_POST["delete"])&&count($_POST["delete"])>0)
	{
		...
		{
			$sql = "update advf_categorias set estado = 'E' where id = $idtodelete";
			$conn->execute($sql);
	...
	if(isset($_GET["c"])&&isset($_GET["id"])&&isset($_GET["v"]))
	{
		if(isset($_GET["r"]) && ($_GET["r"])) {
			switch($_GET["r"]) {
				case 16:
					$record_rights = $conn->execute("select " . $_GET["c"] . " from advf_categorias where id = " . $_GET["id"]);
		...
		$sql = "update advf_categorias set ".$_GET["c"]."='".$_GET["v"]."' where id = ".$_GET["id"];
		$conn->execute($sql);
	...
	if(isset($_GET["a"]))
	{
		if(ereg("Desc",$_GET["a"]))
		{
			$buf = split("Desc",$_GET["a"]);
			// Me tengo que fijar si $buf[0] esta en $orden, si es asi, le tengo que cambiar el asc por el desc
			if(ereg($buf[0],$_GET["orden"]))
			{
				// Esta
				$_GET["orden"] = ereg_replace($buf[0]."(Asc|)",$buf[0]."Desc",$_GET["orden"]);
			}else
			{
				// No esta
				$_GET["orden"] .= $_GET["a"];
			}
		}elseif(ereg("Asc",$_GET["a"]))
		{
			$buf = split("Asc",$_GET["a"]);
			if(ereg($buf[0],$_GET["orden"]))
			{
				// Esta
				$_GET["orden"] = ereg_replace($buf[0]."Desc",$buf[0]."Asc",$_GET["orden"]);
			}else
			{
				// No esta
				$_GET["orden"] .= $_GET["a"];
	...
	if(isset($_GET["filtros"])&&$_GET["filtros"]!="")
	{
		...
		$campos = array("id","nombre","padre");
		$buf = split(" ",$_GET["filtros"]);
		$cond = "";
	...
	{
		$rights_select = " and " . $usr->_id . " in (select admin_auditoria.usuario_id from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = advf_categorias.id and admin_auditoria.fecha in (select max(fecha) from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = advf_categorias.id))";
	}else{
		$rights_select = "";
	}
	...
	{
		$rights_select .= " and advf_categorias.estado != 'E'";
	}else{
		$rights_select .= "";
	}

	// Sacamos la cantidad de elementos que tiene esta tabla.
	$sql = "select count(*) as cuantos from advf_categorias where 1=1 ".($cond!=""?"and $cond":"").($rights_select!=""?"$rights_select":"");
	$rs = $conn->execute($sql);
	...
	$sql = "select id,id,nombre,padre,activo,estado from advf_categorias where 1=1 ".($cond!=""?"and $cond":"").($rights_select!=""?"$rights_select":"")." ".(strlen($orden)>0?"order by ".$orden:"")." limit $current,$cant ";
	$rs = $conn->execute($sql);
...
<div class="why" id="outerDiv">
	...
	<table width="780" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center" class="Title" style="padding-bottom:10px;"><?=$TITULO_SITE?> - Categorías Multimedia</td>
      </tr>
    </table>
    <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" style="background-image:url(images/separador_h8.gif); background-repeat:repeat-x; background-position:bottom;">
      ...
      <tr>
				  ...
				  <tr> 
					<td height="24" class="arial11" style="padding-bottom:8px;"> 
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td colspan="2" class="arial12" style="padding-bottom:4px;"><strong>Buscar por:</strong> ID, Nombre, Padre,</td>
                          </tr>
                          <tr>
        ...
        <td align="left" bgcolor="#EEEEEE" style="background-image:url(images/separador_v.gif); background-repeat:repeat-y; background-position:left; padding-left:15px;"><input name="imageField" type="image" id="imageField" onclick='document.location="?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif">
          <br /><br />
          <?=($usr->checkUserRights(2) != 'disabled')? '<input name="imageField" type="image" id="imageField" onclick=\'document.location="g_categorias_edit.php";\' src="images/btn_nuevo.gif">':''; ?></td>
        ...
        <td width="780" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="5" style="border-collapse: collapse;">
            <tr class="tablaOscuro">
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("id"."Desc",$_GET["orden"]))
					{
						$elem = "id"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("id"."Asc",$_GET["orden"]))
					{
						$elem = "id";
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>ID</a>&nbsp;<?=$imaOrden;?></strong></div></td>
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("nombre"."Desc",$_GET["orden"]))
					{
						$elem = "nombre"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("nombre"."Asc",$_GET["orden"]))
					{
						$elem = "nombre";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Nombre</a>&nbsp;<?=$imaOrden;?></strong></div></td>
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("padre"."Desc",$_GET["orden"]))
					{
						$elem = "padre"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("padre"."Asc",$_GET["orden"]))
					{
						$elem = "padre";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Padre</a>&nbsp;<?=$imaOrden;?></strong></div></td>
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("activo"."Desc",$_GET["orden"]))
					{
						$elem = "activo"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("activo"."Asc",$_GET["orden"]))
					{
						$elem = "activo";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("estado"."Desc",$_GET["orden"]))
					{
						$elem = "estado"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("estado"."Asc",$_GET["orden"]))
					{
						$elem = "estado";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Estado</a>&nbsp;<?=$imaOrden;?></strong></div></td>
          ...
          <td height="19" class="textooferta"><div align="center">
            ...
            <td height="19" class="textooferta" align='center'><a href='g_categorias_edit.php?menu=<?=$usr->_menu;?>&id=<?=$rs->field("id");?>' class="textomediano"> <img alt='Edit' border=0 src="images/icon_editar.gif"> </a> </td>
            
            
            <td class="textooferta" align="center"><span class="textomediano"><?=($rs->field("id")<>""?$rs->field("id"):"&nbsp;");?></span> </td>
            
            
            
            <td class="textooferta" align="center"><span class="textomediano"><?=($rs->field("nombre")<>""?$rs->field("nombre"):"&nbsp;");?></span> </td>
            
            
            
            <td class="textooferta" align="center"><span class="textomediano">
			<?
			$aux = $conn->execute("SELECT id, nombre FROM advf_categorias WHERE id = '".$rs->field("padre")."'");
			if(!$aux->eof)
			{?><?=$aux->field("nombre")?><?}?>
			</span> </td>
            ...
            <td class="textooferta" align="center"><span class="textomediano"><select onchange='Cambiar("activo",document.getElementById("activo<?=$rs->field("id");?>").options[document.getElementById("activo<?=$rs->field("id");?>").options.selectedIndex].value,<?=$rs->field("id");?>,16);' id='activo<?=$rs->field("id");?>' name='activo<?=$rs->field("id");?>' class='comun' <? print($usr->checkUserRights(16)); ?>><option value='S' <?=($rs->field("activo") == 'S')?"selected":"";?>>Si</option><option value='N' <?=($rs->field("activo") == 'N')?"selected":"";?>>No</option></select></span> </td>
            
            
            
            <td class="textooferta" align="center"><span class="textomediano"><select onchange='Cambiar("estado",document.getElementById("estado<?=$rs->field("id");?>").options[document.getElementById("estado<?=$rs->field("id");?>").options.selectedIndex].value,<?=$rs->field("id");?>,32);' id='estado<?=$rs->field("id");?>' name='estado<?=$rs->field("id");?>' class='comun' <? print($usr->checkUserRights(32)); ?>><option value='A' <?=($rs->field("estado") == 'A')?"selected":"";?>>Aprobado</option><option value='P' <?=($rs->field("estado") == 'P')?"selected":"";?>>Pendiente</option><option value='D' <?=($rs->field("estado") == 'D')?"selected":"";?>>Desaprobado</option><option value='E' <?=($rs->field("estado") == 'E')?"selected":"";?>>Eliminado</option></select></span> </td>

<? 
	...
	if(is_array($_POST["delete"])&&count($_POST["delete"])>0)
	{
		...
		{
			$sql = "update admin_usuarios set estado = 'E' where id = $idtodelete";
			$conn->execute($sql);
	...
	if(isset($_GET["c"])&&isset($_GET["id"])&&isset($_GET["v"]))
	{
		if(isset($_GET["r"]) && ($_GET["r"])) {
			switch($_GET["r"]) {
				case 16:
					$record_rights = $conn->execute("select " . $_GET["c"] . " from admin_usuarios where id = " . $_GET["id"]);
		...
		$sql = "update admin_usuarios set ".$_GET["c"]."='".$_GET["v"]."' where id = ".$_GET["id"];
		$conn->execute($sql);
	...
	if(isset($_GET["filtros"])&&$_GET["filtros"]!="")
	{
		...
		$campos = array("nombre","apellido","usuario");
	...
	{
		$rights_select = " and " . $usr->_id . " in (select admin_auditoria.usuario_id from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = admin_usuarios.id and admin_auditoria.fecha in (select max(fecha) from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = admin_usuarios.id))";
	}else{
		$rights_select = "";
	}
	...
	{
		$rights_select .= " and admin_usuarios.estado != 'E'";
	}else{
		$rights_select .= "";
	}

	// Sacamos la cantidad de elementos que tiene esta tabla.
	$sql = "select count(*) as cuantos from admin_usuarios where id > 1 AND 1=1 ".($cond!=""?"and $cond":"").($rights_select!=""?"$rights_select":"");
	$rs = $conn->execute($sql);
	...
	$sql = "select id,nombre,apellido,usuario,activo from admin_usuarios where id >1 AND  1=1 ".($cond!=""?"and ( $cond )":"").($rights_select!=""?"$rights_select":"")." ".(strlen($orden)>0?"order by ".$orden:"")." limit $current,$cant ";
    ...
    <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" style="background-image:url(images/separador_h8.gif); background-repeat:repeat-x; background-position:bottom;">
      ...
      <tr>
				  ...
				  <tr> 
					<td height="24" class="arial11" style="padding-bottom:8px;"> 
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td colspan="2" class="arial12" style="padding-bottom:4px;"><strong>Buscar por:</strong> Nombre, Apellido, Usuario,</td>
                          </tr>
                          <tr>
        ...
        <td align="left" bgcolor="#EEEEEE" style="background-image:url(images/separador_v.gif); background-repeat:repeat-y; background-position:left; padding-left:15px;"><input name="imageField" type="image" id="imageField" onclick='document.location="?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif">
          <br /><br />
          <?=($usr->checkUserRights(2) != 'disabled')? '<input name="imageField" type="image" id="imageField" onclick=\'document.location="g_usuarios_admin_edit.php";\' src="images/btn_nuevo.gif">':''; ?></td>
        ...
        <td width="780" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="5" style="border-collapse: collapse;">
            <tr class="tablaOscuro">
              ...
              <td height="24" class="titulooferta">&nbsp;</td>
			   ...
			   <td height="24" align="center" class="titulooferta"> &nbsp;
                </td>
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("apellido"."Desc",$_GET["orden"]))
					{
						$elem = "apellido"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("apellido"."Asc",$_GET["orden"]))
					{
						$elem = "apellido";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Apellido</a>&nbsp;<?=$imaOrden;?></strong></div></td>
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					if(ereg("usuario"."Desc",$_GET["orden"]))
					{
						$elem = "usuario"."Asc";
						$imaOrden = "<img src='images/flecha_down.gif' >";

					}elseif(ereg("usuario"."Asc",$_GET["orden"]))
					{
						$elem = "usuario";
						$imaOrden = "<img src='images/flecha_up.gif'>";
					}else
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Usuario</a>&nbsp;<?=$imaOrden;?></strong></div></td>
          ...
          <td height="19" class="textooferta"><div align="center">
            ...
            <td height="19" class="textooferta" align='center'><a href='g_usuarios_admin_edit.php?menu=<?=$usr->_menu;?>&id=<?=$rs->field("id");?>' class="textomediano"> <img alt='Edit' border=0 src="images/icon_editar.gif"> </a> </td>
            
            <td class="textooferta" >
				<span class="textomediano"><a href='g_usuarios_admin_permisos.php?id=<?=$rs->field("id");?>'><img src='images/icon_permisos.gif' border='0' alt='Edit users permissions'></a></span>
            </td>
            ...
            <td class="textooferta" align="center"><span class="textomediano"><?=($rs->field("apellido")<>""?$rs->field("apellido"):"&nbsp;");?></span> </td>
            
            
            
            <td class="textooferta" align="center"><span class="textomediano"><?=($rs->field("usuario")<>""?$rs->field("usuario"):"&nbsp;");?></span> </td>

    ...
    var MapaTelam = function(element, options)
    {
        ...
        this.Inicializate = function()
        {
            ...
            google.maps.event.addListener(map, 'zoom_changed', function() {
                if (false === obj.getConfig().onChangeZoom(map.getZoom()))
                    return;
            });

            google.maps.event.addListener(map, 'maptypeid_changed', function() {
                if (false === obj.getConfig().onChangeMapType(returnMapType(map.getMapTypeId())))
                    return;
            });
        ...
        InputBuscador.keypress(function(event) {
            timeoutHnd = setTimeout(obj.Geocode, 500)
        });
        ...
        this.AcceptAddMarkerButton = function(button)
        {
            google.maps.event.addListener(map, button, function(e) {
                ...
                if (false === obj.getConfig().beforeSelected(map, e.latLng))
                    return;
        ...
        this.AddMarker = function(lat, lng)
        {
            ...
            contadorMarker++;
            if (false === obj.getConfig().beforeSelected(map, myLatLng))
                return;
        }

        this.AddMarkerCustomIcon = function(lat, lng, icon)
        {
            var myLatLng = new google.maps.LatLng(lat, lng);
            var marker = new google.maps.Marker({
                position: myLatLng,

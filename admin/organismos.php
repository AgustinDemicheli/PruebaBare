<? 
	...
	if(is_array($_POST["delete"])&&count($_POST["delete"])>0)
	{
		...
		{
			$sql = "update organismos set estado = 'E' where id = $idtodelete";
			$conn->execute($sql);
	...
	if(isset($_GET["c"])&&isset($_GET["id"])&&isset($_GET["v"]))
	{
		if(isset($_GET["r"]) && ($_GET["r"])) {
			switch($_GET["r"]) {
				case 16:
					$record_rights = $conn->execute("select " . $_GET["c"] . " from organismos where id = " . $_GET["id"]);
		...
		$sql = "update organismos set ".$_GET["c"]."='".$_GET["v"]."' where id = ".$_GET["id"];
		$conn->execute($sql);
	...
	{
		$rights_select = " and " . $usr->_id . " in (select admin_auditoria.usuario_id from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = organismos.id and admin_auditoria.fecha in (select max(fecha) from admin_auditoria where admin_auditoria.menu_id = '" . $usr->_menu . "' and admin_auditoria.contenido_id = organismos.id))";
	}else{
		$rights_select = "";
	}
	...
	{
		$rights_select .= " and organismos.estado != 'E'";
	}else{
		$rights_select .= "";
	}

	// Sacamos la cantidad de elementos que tiene esta tabla.
	$sql = "select count(*) as cuantos from organismos where 1=1 ".($cond!=""?"and $cond":"").($rights_select!=""?"$rights_select":"");
	$rs = $conn->execute($sql);
	...
	$sql = "select id,nombre,activo from organismos where 1=1 ".($cond!=""?"and $cond":"").($rights_select!=""?"$rights_select":"")." ".(strlen($orden)>0?"order by ".$orden:"")." limit $current,$cant ";
	$rs = $conn->execute($sql);
    ...
    <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" style="background-image:url(images/separador_h8.gif); background-repeat:repeat-x; background-position:bottom;">
      ...
      <tr>
				  ...
				  <tr> 
					<td height="24" class="arial11" style="padding-bottom:8px;"> 
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td colspan="2" class="arial12" style="padding-bottom:4px;"><strong>Buscar por:</strong> Nombre Organismo,</td>
                          </tr>
                          <tr>
        ...
        <td align="left" bgcolor="#EEEEEE" style="background-image:url(images/separador_v.gif); background-repeat:repeat-y; background-position:left; padding-left:15px;"><input name="imageField" type="image" id="imageField" onclick='document.location="?=menu=<?=$usr->_menu;?>";' src="images/btn_volver.gif">
          <br /><br />
          <?=($usr->checkUserRights(2) != 'disabled')? '<input name="imageField" type="image" id="imageField" onclick=\'document.location="organismos_edit.php";\' src="images/btn_nuevo.gif">':''; ?></td>
        ...
        <td width="780" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="5" style="border-collapse: collapse;">
            <tr class="tablaOscuro">
              ...
              <td height="24" align="center" class="arial12"><div align="center"><strong><?
					...
					?><a href='<?=$_SERVER["PHP_SELF"];?>?orden=<?=$_GET["orden"];?>&a=<?=$elem;?>&filtros=<?=$_GET["filtros"];?>&filtros_p=<?=$_GET["filtros_p"];?>'>Nombre Organismo</a>&nbsp;<?=$imaOrden;?></strong></div></td>
          ...
          <td height="19" class="textooferta"><div align="center">
            ...
            <td height="19" class="textooferta" align='center'><a href='organismos_edit.php?menu=<?=$usr->_menu;?>&id=<?=$rs->field("id");?>' class="textomediano"> <img alt='Edit' border=0 src="images/icon_editar.gif"> </a> </td>

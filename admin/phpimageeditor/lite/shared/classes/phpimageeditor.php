...
*/
		...
		function PHPImageEditor()
		{
			...
			if ($_GET[$this->httpImageKey] != NULL)
			{
				...
				$idFoto = strip_tags($_GET[$this->httpImageKey]);
			...
			if (!$info)
			{
				$this->errorMessages[] = $this->srcEdit;
				return;
			}
		...
		function LoadLanguage()
		{
			...
			if (file_exists($tryLanguage))
				...
				$this->texts = PIE_GetTexts("language/".DEFUALT_LANGUAGE.".ini");
		...
		function Action($actionType)
		{
			...
			{
				global $conn; global $app_path;

				$aux = $conn->execute("SELECT catID, advTitulo FROM advf WHERE advID = '".$this->idFotoGet."'");
				if(!$aux->eof)
				{
					$categ = $aux->field("catID");
					$advTitulo = $aux->field("advTitulo");
				}
				else
					$categ = 0;

				$uniq = uniqid();
				$nueva_img = "advf/imagenes/editadas/".$uniq.".jpg";

				$conn->execute("INSERT INTO advf(advTipo, advTitulo, advFecha, advLink, catID) VALUES('F','".$_REQUEST["newTitle"]."',NOW(),'".$nueva_img."','".$categ."')");
				
				$nuevo_id = $conn->UltimoId();
				$nueva_ruta = $app_path."advf/imagenes/editadas/".$uniq.".jpg";

				$this->SaveImage($this->resourceWorkWith, $nueva_ruta, $this->mimeType);

				//$this->SaveImage($this->resourceWorkWith, $this->srcEdit, $this->mimeType);
				unlink($this->srcOriginal);
				unlink($this->srcPng);
				unlink($this->srcWorkWith);
				PIE_DeleteOldImages(IMAGE_ORIGINAL_PATH);
				PIE_DeleteOldImages(IMAGE_PNG_PATH);
				PIE_DeleteOldImages(IMAGE_WORK_WITH_PATH);
				...
				PIE_Echo('<script language="javascript" type="text/javascript">'
								.$reloadParentBrowser.'
								try{
									window.opener.actualizarFoto('.$nuevo_id .',"'. LAUNCHER. '"); 
								}catch(e){
		...
		function GetFormAction()
		{
			...
			return "index.php?launcher=".LAUNCHER."&".$joomlaIsAdmin.$this->httpImageKey."=".$this->idFotoGet;
		}
